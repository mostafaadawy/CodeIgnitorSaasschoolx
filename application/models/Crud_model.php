...
//       echo $this->email->print_debugger();
   ...
   function send_homework_notify($class  ,$section  ,$sub ="",$title = "",$content = "")
    {
		...
		$class_entries = sizeof($class);
		for($i = 0; $i < $class_entries; $i++) 
            {
				$students = $this->db->get_where('enroll', array('class_id' => $class[$i], 'section_id' => $section[$i], 'year' => $year))->result_array();
				$mails = array();
				$from = $this->db->get_where('settings', array('type' => 'system_email'))->row()->description;
				$system_name = $this->db->get_where('settings', array('type' => 'syste_title'))->row()->description;
    ...
    {
		$startdate =  implode('', array_reverse(explode('/', $this->input->post('availablefrom')))) .implode('',explode(':',$this->input->post('clock_start')));
		$enddate   = implode('', array_reverse(explode('/', $this->input->post('availableto')))) .implode('',explode(':',$this->input->post('clock_end')));
		if ($enddate >= $startdate)
		{
			...
			$class_id_s   = $this->input->post('class_id');
		    $section_id_s = $this->input->post('section_id');
		    $class_entries = sizeof($class_id_s);
		 for($i = 0; $i < $class_entries; $i++) 
            {
				$data['class_id'] = $class_id_s[$i];
				$data['section_id'] = $section_id_s[$i];
	...
	function update_exam($exam_code) {
		//$startdate = $this->input->post('availablefrom').' '.$this->input->post('clock_start');
		//$enddate   = $this->input->post('availableto').' '.$this->input->post('clock_end').implode('',explode(':',).$this->input->post('clock_start'));
		$startdate =  implode('', array_reverse(explode('/', $this->input->post('availablefrom')))) .implode('',explode(':',$this->input->post('clock_start')));
		$enddate   = implode('', array_reverse(explode('/', $this->input->post('availableto')))) .implode('',explode(':',$this->input->post('clock_end')));
	 ...
	 function create_event() 
    {
		...
		if ($enddate >= $startdate)
		{
			...
			move_uploaded_file($_FILES['userfile']['tmp_name'], $subdomain.'uploads/news_images/' . $news_code . '.jpg');
			return true;
		}
		else
			return false;
     ...
     function create_post() 
     {
        ...
        move_uploaded_file($_FILES["file_name"]["tmp_name"], $subdomain."uploads/forum/" . $_FILES["file_name"]["name"]);
        return $post_code;
    }
    ...
    function homework_create() 
    {
        ...
        move_uploaded_file($_FILES["file_name"]["tmp_name"], $subdomain."uploads/homework/" . $_FILES["file_name"]["name"]);
    ...
    function public_files($id)
    {
        ...
        $this->db->insert('homework', $data);
        $homework_code = $this->db->get_where('homework', array('homework_id' => $this->db->insert_id()))->row()->homework_code;
        move_uploaded_file($_FILES["file_name"]["tmp_name"], $subdomain . "uploads/public/" . $_FILES["file_name"]["name"]);
    }   
   ...
   function send_reply_group_message($message_thread_code) 
   {
    ...
    function admin_create() {
        ...
        move_uploaded_file($_FILES['userfile']['tmp_name'], $subdomain.'uploads/admin_image/' . $new_admin_id . '.jpg');
    }

    function admin_edit($admin_id) 
    {
		$subdomain = $this->db->get_where('settings', array('type' => 'system_name'))->row()->description;
		$username = $this->db->get_where('admin' , array('admin_id' => $admin_id))->row()->username;
	...
	function teacher_edit($teacher_id) 
    {
			...
			$username = $this->db->get('teacher')->row()->username;
		    //$username  = $this->db->get_where('teacher' , array('teacher_id' => $teacher_id))->row()->username;
			$new_username = $this->input->post('username');
			if($username === $new_username || $this->users_model->validate_username($new_username))
			{
				$data['name']        = $this->input->post('name');
    ...
    function get_grade($mark_obtained, $class_id , $subject_id) 
    {
        $query = $this->db->get_where('grade' , array('class_id'=>$class_id , 'subject_id' => $subject_id));
    ...
    function get_image_url($type = '', $id = '') {
		$subdomain = $this->db->get_where('settings', array('type' => 'system_name'))->row()->description;
        if (file_exists($subdomain.'uploads/' . $type . '_image/' . $id . '.jpg'))
            $image_url = base_url() . $subdomain . 'uploads/' . $type . '_image/' . $id . '.jpg';
        else
            $image_url = base_url() . $subdomain . 'uploads/user.jpg';
    ...
    {
		$subdomain = $this->db->get_where('settings', array('type' => 'system_name'))->row()->description;
         if (file_exists($subdomain . 'uploads/screen/' . $id . '.jpg'))
            $image_url = base_url() . $subdomain . 'uploads/screen/' . $id . '.jpg';
        else $image_url = base_url() . $subdomain . 'uploads/user.jpg';
    ...
    function save_study_material_info()
    {
        ...
        move_uploaded_file($_FILES["file_name"]["tmp_name"], $subdomain . "uploads/document/" . $_FILES["file_name"]["name"]);
    }
    ...
    function teacher_files()
    {
        ...
        if (in_array(strtolower($fileParts['extension']), $fileTypes)) 
        {
            ...
            move_uploaded_file($_FILES["file_name"]["tmp_name"], $subdomain . "uploads/teacher_files/" . $_FILES["file_name"]["name"]);
        } 
        else 
        {
            $this->session->set_flashdata('error_message' , "No fue posible subir el archivo debido a que la extensión no esta permitida.");
    ...
    function import_db()
    {
        ...
        move_uploaded_file($_FILES["file_name"]["tmp_name"], $subdomain."uploads/" . $_FILES["file_name"]["name"]);
        $filename = $subdomain."uploads/".$file_n;
        ...
        unlink($subdomain."uploads/" . $file_n);
     ...
     function create_notice_message($notice_code = '') 
    {
        ...
        if ( $_FILES['userfile']['name'] != '')
            move_uploaded_file($_FILES['userfile']['tmp_name'], $subdomain.'uploads/notice_message_file/' . $_FILES['userfile']['name']);
    ...
    function create_report() 
    {
        ...
        move_uploaded_file($_FILES['file']['tmp_name'], $subdomain.'uploads/reportes_alumnos/' . $_FILES['file']['name']);
    }
