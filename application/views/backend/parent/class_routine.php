						...
						?>
							...
							<table class="table table-schedule table-hover" cellpadding="0" cellspacing="0">
								...
								<?php
									...
									$students = $this->db->get_where('subject', array('subject_id' => $row3['subject_id'], 'year' => $running_year))->row()->students;
									$type     = $this->db->get_where('subject', array('subject_id' => $row3['subject_id'], 'year' => $running_year))->row()->type;
									if(!$type ==1  || strpos($students ,'"'.$row2['student_id'].'"' )!= false ):
			                        $teacher_id = $this->db->get_where('subject', array('subject_id' => $row3['subject_id']))->row()->teacher_id;
			                	?>
									<td style="text-align:center">
									<?php
									 if ($row3['time_start_min'] == 0 && $row3['time_end_min'] == 0) 
									echo $row3['time_start'].":00" . '-' . $row3['time_end'].":00";
									if ($row3['time_start_min'] != 0 || $row3['time_end_min'] != 0)
									echo $row3['time_start'].':'.$row3['time_start_min'].'-'.$row3['time_end'].':'.$row3['time_end_min'];
			                    ?>
			                    	<br><b><?php echo $this->crud_model->get_subject_name_by_id($row3['subject_id']);?></b><br><small><?php echo $this->db->get_where('teacher', array('teacher_id' => $teacher_id))->row()->name;?></small>
									</td>
