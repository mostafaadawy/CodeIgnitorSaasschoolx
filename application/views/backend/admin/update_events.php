      ...
      <div class="element-box shadow lined-primary">
        <?php echo form_open(base_url() . 'admin/update/event/'.$row['news_code'], array('enctype' => 'multipart/form-data')); ?>
          ...
          <div class="row">
              <div class="col-sm-6">
                <div class="form-group">
                  <label for=""> <?php echo get_phrase('from');?></label><input class="form-control" value="<?php echo $row['from_'];?>" required="" name="from" id="datefrom" type="text">
                </div>
              </div>
              <div class="col-sm-6">
                <div class="form-group">
                  <label for=""> <?php echo get_phrase('until');?></label><input class="form-control" value="<?php echo $row['to_'];?>" name="to" id="dateto" required type="text">
                </div>
              </div>
            </div>
		  <div class="form-group">			<label for=""> <?php echo get_phrase('featured_image');?></label>			  <div class="newsfe" style="max-width:500px;">				<button type="button" class="change-cover">					<i class="font-icon picons-thin-icon-thin-0617_picture_image_photo"></i>					<?php echo get_phrase('upload');?>					<input accept="image/x-png,image/gif,image/jpeg" id="imgpre" type="file"/ name="userfile">				</button>				<img width="100%" id="ava" src="<?php echo base_url().$subdomain;?>uploads/img_pre.jpg">			</div>			 </div>
